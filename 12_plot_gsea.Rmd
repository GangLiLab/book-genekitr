...
[GSEA analysis](#gsea-basic-usage) returns a list result, and there are two ways of visualization:
...
> For more details, please refer to [chapter 9](#gsea-basic-usage)
...
If users want to show specific pathways or genes, just pass them to the corresponding parameters: 
- `show_pathway`: IDs in the GSEA result
- `show_gene`: IDs in the GSEA result
...
If users want to show a bunch of selected pathways, `plotGSEA` borrows some code from `r  Biocpkg("fgsea")`.
...
> More plotting details are available at [chapter 11](#plot-ora-1)
