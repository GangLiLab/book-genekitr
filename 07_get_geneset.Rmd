...
To facilitate GEA, I developed the package `r CRANpkg("geneset")`, which provides a comprehensive list of monthly updated gene set (GS) libraries.

## Geneset package introduction 

The R package includes curated gene sets from [GO](http://geneontology.org/) (BP, CC and MF), [KEGG](https://www.kegg.jp/kegg/) (pathway, module, enzyme, network, drug and disease), [WikiPathway](https://wikipathways.org/), [MsigDb](https://www.gsea-msigdb.org/gsea/msigdb/), [EnrichrDb](https://maayanlab.cloud/Enrichr/), [Reactome](https://reactome.org/), [MeSH](https://www.ncbi.nlm.nih.gov/mesh/), [DisGeNET](https://www.disgenet.org/), [Disease Ontology](https://disease-ontology.org/) (DO), [Network of Cancer Gene](http://ncg.kcl.ac.uk/) (NCG) (version 6 and v7) and [COVID-19](https://maayanlab.cloud/covid19/). 
...
- WikiPathway supports 16 species
- Reactome supports 11 species
- EnrichrDB supports 5 species
- Disease-related sets only support human (DO, NCG, DisGeNET and COVID-19)
...
GO is short for [Gene Ontology](http://geneontology.org). GO analysis aims to find the associations between gene products and GO terms, which has three domains:
...
GO terms are organized in a directed acyclic graph with a parent-child relationship.

> For a more comprehensive introduction to GO, you may visit: https://advaitabio.com/faq-items/understanding-gene-ontology/ OR http://geneontology.org/docs/ontology-documentation/
...
The result is a list that includes four parts:

- `gene set` (formatted as a data frame): two columns containing GO term IDs and matched gene IDs
- `geneset_name` (formatted as a data frame): two columns containing GO term IDs and matched GO descriptions
...
- `category`: MeSH descriptor categories. For more details, refer to: [How to use MeSH-related Packages](https://rdrr.io/bioc/meshr/f/inst/doc/MeSH.pdf)
...
[MsigDB categories](http://www.gsea-msigdb.org/gsea/msigdb/collections.jsp) is the best GSEA partner, which has 9 major collections and several sub-collections comprising 32880 gene sets: 
...
- `gene set` (formatted as a data frame): two columns containing pathway IDs and matched gene IDs
- `geneset_name`: NA (because the pathway IDs and names are the same, so we simply ignore them)
- `organism`: stores `org` information
- `type`: stores `ont` information
...
You only need to input the organism name.
...
### Enrichr introduction

Enrichr is a comprehensive resource of curated gene sets and a search engine that accumulates biological knowledge for further biological discoveries.
...
> For now, we support human disease annotation data from: Disease Ontology (DO), DisGeNET, Network of Cancer Gene (NCG) version 6 and v7, and COVID-19

You only need to input the source name from "do", "ncg_v7", "ncg_v6", "disgenet" and "covid19".
...
- `disgenet`: [DisGeNET](https://www.disgenet.org/) is a discovery platform containing one of the largest publicly available collections of genes and variants associated with human diseases.
DisGeNET integrates data from expert curated repositories, GWAS catalogues, animal models and the scientific literature. DisGeNET data are homogeneously annotated with controlled vocabularies and community-driven ontologies. Additionally, several original metrics are provided to assist in the prioritization of genotype-phenotype relationships.
