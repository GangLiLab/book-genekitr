...
It is worth mentioning that **almost all plots are based on `r CRANpkg("ggplot2")`** and the `plot_theme` function can easily change their border, legend, label etc., which helps users make their own plots.

## Get ORA result

> For more details, please refer to [chapter 8](#ora-basic-usage)
...
By the way, the "q-value" stands for the "false discovery rate (FDR)" method.
...
After analyzing group enrichment analysis for [ORA](#ora-group-enrichment), we can use `dotplot` to show the result:
...
(ref:lolliplotCap) **Lollipop plot of enrichment analysis.** default (A), selected genes (B)
...
The heatmap plot shows interactions between enriched terms and their genes.
If fold change is given, the heatmap will add color for up- and down-regulated genes.
...
Inspired by [GOplot](https://wencke.github.io/), the chord plot is reproduced using `r CRANpkg("ggplot2")` and it shows content similar to the heatmap plot.
...
(ref:chordCap) **Chord plot of enrichment analysis.** selected genes (A), selected genes with logFC value (B).
...
Inspired by `r CRANpkg("ComplexUpset")`, the upset plot shows the association between genes and enriched terms. Unlike a common Venn diagram, it can emphasize the complex relationships among many gene sets.
...
- GO analysis results can use five semantic similarity methods to calculate the similarity between nodes: "Resnik", "Lin", "Rel", "Jiang", "Wang", as well as the "JC" (Jaccard\u2019s similarity index) method.
- KEGG only supports the "JC" method.

Users can define the `layout` argument derived from `r CRANpkg("ggraph")`, including "nicely" (default), "circle", "dh", "drl", "fr", "graphopt", "grid","lgl", "kk", "mds", "randomly", "star" etc.
...
(ref:networkCap) **Network plot of enrichment analysis.** JC method and nicely layout (A, default), circle layout (B), grid layout and Wang method (C).
...
> To visualize more than one ontology of GO in one plot, users can use the `wego plot`
...
**NOTICE:** `plotEnrich` has a parameter `n_term` for the WEGO plot, which specifies the number of terms. If you want to plot all terms, just set `n_term` higher.
...
Here the map plot utilized `r Biocpkg("GOSemSim")` to extract parent and child terms, and also utilized `r CRANpkg("ggraph")` and `r CRANpkg("igraph")` to draw with the default layout "sugiyama".
...
Inspired by `r Biocpkg("rrvgo")`, `genekitr` borrows the main code to cluster GO terms. It can also use five methods to calculate the similarity between terms.
...
According to the [rrvgo vignettes](http://bioconductor.org/packages/release/bioc/vignettes/rrvgo/inst/doc/rrvgo.html), the tangram plot is a space-filling visualization of hierarchical structures. The terms are grouped and colored based on their parent, and the space used by the term is proportional to the score.  
...
There are two visualization types: 
...
(ref:twogroup2Scap) Visualize two groups separately when `plot_type = "two"`.

(ref:twogroup2Cap) **Visualize two groups separately when `plot_type = "two"`.** 
