...
`plotVenn` supports two types: Venn (default) and UpSet. Besides, it also supports customization of text size, alpha degree, colors, and other parameters via `plot_theme`.
...
The UpSet plot is better suited for more than four groups of genes:
...
After finishing differential expression analysis, we can obtain differentially expressed genes (DEGs).
Volcano plots are widely used to visualize these up- and down-regulated genes.

The example data is from a microarray differential expression analysis:
...
- `logFC_cutoff`: cutoff for log2 (fold change), default is 1, which is actually a two-fold change
- `up_color`: color for up-regulated genes
- `down_color`: color for down-regulated genes
- `show_gene`: show some gene names, default is none. We suggest renaming the column with these genes as "gene".
...
You can hide all x/y-axis labels and increase `main_text_size` to enlarge the gene text, then export the figure to PowerPoint to add labels manually.
