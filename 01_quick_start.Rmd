...
> This section helps you quickly get a sense of how each function works. Click on the titles below to view more details.
...
# supports both hg19 and hg38 genome build versions
v19 <- genInfo(id, org = "hs",hgVersion = "v19")
v38 <- genInfo(id, org = "hs",hgVersion = "v38")
```
...
# set "num" to a larger value to get all the records
...
> To facilitate gene enrichment analysis, I developed `r CRANpkg("geneset")`.
...
`geneList` is a pre-ranked gene list ordered in decreasing order. Input IDs can be Entrez, Ensembl, Symbol, or UniProt IDs.
...
### Import Panther results from the GeneOntology website
```{r eval=FALSE}
# Here we save the Panther result as "analysis.txt"
...
kk <- gseKEGG(geneList     = geneList,
               ...
               pvalueCutoff = 0.05,
               verbose      = FALSE)
## Other ORA results from clusterProfiler 
...
## Convert clusterProfiler results into genekitr format
